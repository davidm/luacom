...
\usepackage{times}
\usepackage[english]{babel}
\usepackage[a4paper=true,pdfsubject="The LuaCOM Manual",pdfauthor="Vinicius Almendra and Renato Cerqueira and Fabio Mascarenhas",pdftitle="The LuaCOM Manual",colorlinks=true,citecolor=black,linkcolor=black,pagecolor=black,urlcolor=black]{hyperref}
...
\section{Features}
  ...
  \begin{itemize}
    ...
    \item fully compatible with Lua 5 and with Lua 4;
...
\section*{Some information about samples}
...
\section{Using The \luacom\ library}
  ...
  library through Lua 5's require/loadlib mechanism. To use dynamic
  loading in Lua 4 you should implement a similar mechanism. There
  are different versions of the \luacom\ binary for the different
  ...
  Lua\footnote{Notice that COM must be initialized in each thread that
...
\verb+lua_open+ and to terminate it only after the last
\verb+lua_close+, otherwise fatal errors may occur.

 Using Lua 5 to dynamically load \luacom\ is simpler. Just call \verb+require("luacom")+
 in your Lua script, and make sure the file \verb+luacom.lua+ is in your \verb+LUA_PATH+
 environment variable, and the Lua and \luacom\ DLLs (\verb+lua-5.0.dll+, \verb+lualib-5.0.dll+ and
...
Microsoft\copyright\ Word\copyright\ is ``\ident{Word.Application}''.
...
%% Extended API table
  ...
  \verb+ViewTypeLib+
    ...
    Internet Explorer\copyright.} \\ \hline
...
\paragraph{Lua 5}
\luacom\ uses the boolean values \verb+true+ and \verb+false+, but
does not works with the older convention (\verb+nil+ and
non-\verb+nil+; see paragraph below).

\paragraph{Lua 4}
This version of Lua uses the \ident{nil} value as false and
non-\ident{nil} values as true. As \luacom\ gives a special meaning
for \ident{nil} values in the parameter list, it can't use \lua\
convention for true and false values; instead, \luacom\ uses the
\ident{C} convention: the true value is a number different from zero
...
If the table does not have a \verb+tocom+ tag method (for Lua 4) or
\verb+__tocom+ metamethod (for Lua 5), \luacom\ first checks if the
...
exception, that may be translated to a \verb+lua_error+ or to a \com\
...
languages like C++ or Visual Basic\copyright . That's true in many
...
Microsoft's\copyright\ MIDL. Notice that all the interfaces must be
...
\item \luacom\ doesn't provide access to \com\ interfaces that doesn't
...
or environments (Visual Basic\copyright, VBA, VBScript\copyright,
...
\subsection{Visual Basic\copyright\ issue}
...
similar way as the are in VBScript\copyright;
...
\item now it's possible to create instances of Microsoft\copyright\
Office\copyright\ applications (Excel\copyright, Powerpoint\copyright\
...
can access Microsoft Internet Explorer\copyright\ object);
...
file all errors that occurr. If the library was compiled with VERBOSE
